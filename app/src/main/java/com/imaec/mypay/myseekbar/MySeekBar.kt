package com.imaec.mypay.myseekbar
...
class MySeekBar : View {
    ...
    constructor(context: Context, attrs: AttributeSet) : super(context, attrs) {
        applyAttributes(context.theme.obtainStyledAttributes(attrs, R.styleable.MySeekBar, 0, 0))
    }
    ...
    private fun applyAttributes(attributes: TypedArray) {
        try {
            startAngle = attributes.getFloat(R.styleable.MySeekBar_startAngleDegrees, startAngle)
            thumbRadius = attributes.getDimension(R.styleable.MySeekBar_thumbRadius, thumbRadius)
            thumbColor = attributes.getColor(R.styleable.MySeekBar_thumbColor, thumbColor)
            val trackGradientArrayId = attributes.getResourceId(R.styleable.MySeekBar_trackGradient, 0)
            if (trackGradientArrayId != 0) {
                trackGradientArray = resources.getIntArray(trackGradientArrayId)
            }

            val trackGradientArrayPositionsResourceId = attributes.getResourceId(R.styleable.MySeekBar_trackGradientPositions, 0)
            ...
            showThumb = attributes.getBoolean(R.styleable.MySeekBar_showThumb, showThumb)
            progressWidth = attributes.getDimension(R.styleable.MySeekBar_progressWidth, progressWidth)
            trackWidth = attributes.getDimension(R.styleable.MySeekBar_trackWidth, trackWidth)
            progress = attributes.getFloat(R.styleable.MySeekBar_progress, 0f)

            val gradientArrayResourceId = attributes.getResourceId(R.styleable.MySeekBar_progressGradient, 0)
            if (gradientArrayResourceId != 0) {
                progressGradientArray = resources.getIntArray(gradientArrayResourceId)
            }
            val gradientArrayPositionsResourceId = attributes.getResourceId(R.styleable.MySeekBar_progressGradientPositions, 0)
            if (gradientArrayPositionsResourceId != 0) {
                val positionsIntArray = resources.getIntArray(gradientArrayPositionsResourceId)
                progressGradientArrayPositions = FloatArray(positionsIntArray.size) { positionsIntArray[it].div(100f) }
            }

            interactive = attributes.getBoolean(R.styleable.MySeekBar_interactive, interactive)
            thumbDrawableId = attributes.getResourceId(R.styleable.MySeekBar_thumbDrawable, 0)
            showProgress = attributes.getBoolean(R.styleable.MySeekBar_showProgress, showProgress)
        } finally {
            attributes.recycle()
        }
    }
